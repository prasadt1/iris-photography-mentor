...
export const MemoryGridSkeleton: React.FC = () => (
  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 animate-pulse" aria-hidden>
    {Array.from({ length: 6 }).map((_, i) => (
      <div key={i} className="rounded-2xl bg-slate-800/60 border border-slate-700 overflow-hidden">
...
export const PracticeCardsSkeleton: React.FC = () => (
  <div className="space-y-4 animate-pulse" aria-hidden>
    {Array.from({ length: 2 }).map((_, i) => (
      <div key={i} className="rounded-xl border border-slate-700 bg-slate-800/50 p-5 space-y-3">
